...
dataname = r'data_improved/yanimproved.csv'
...
#cluster using sc
KNN_for_distance = 6#6
KNN_for_neighbord = 12#12
...
for j in range(length):
    ...
    c1=c1[:,0:len(np.argwhere(c1[1,:]<10000))]  #used for Dijkstra distance change   shape(2*M) index0 means target  index1 means distance
...
aaa= np.concatenate([aaa] * 90, axis=0)
aaa=aaa.reshape(90,90)
...
sc1 = SpectralClustering(n_clusters=n,affinity='nearest_neighbors',n_neighbors=KNN_for_neighbord)
print('SC KNN ACC:', acc(y, sc1.fit_predict(x1)))
...
fh = open('performanceyanimproved.txt', 'w', encoding='utf-8')
fh.write(c)
fh.close()